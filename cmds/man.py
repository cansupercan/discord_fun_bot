...
class man(Cog_Extension):
    ...
    @commands.command()
    async def pingd(self,ctx):
        dt1 = datetime.utcnow().replace(tzinfo=timezone.utc)
        dt = dt1.astimezone(timezone(timedelta(hours=8))) # 轉換時區 -> 東八區
        embed=discord.Embed(title=("🤖"+jdata["bot_name"]+"的PING:"),  color=0x4DFFFF, timestamp=dt) 
        embed.add_field(name="", value=f'{round(self.bot.latency*1000)}ms', inline=True)
        embed.set_footer(icon_url=jdata["bot_head_url"],text=jdata["bot_name"])
        await ctx.send(embed=embed)
    ...
    async def invite(self,ctx):
        dt1 = datetime.utcnow().replace(tzinfo=timezone.utc)
        dt = dt1.astimezone(timezone(timedelta(hours=8))) # 轉換時區 -> 東八區
        embed=discord.Embed(title=("🤖"+jdata["bot_name"]+"的邀請連結:"),  color=0x4DFFFF, timestamp=dt) 
        embed.add_field(name="", value=jdata["invite_url"], inline=True)
        embed.set_footer(icon_url=jdata["bot_head_url"],text=jdata["bot_name"])
        embed.set_image(url="https://i.imgur.com/5piS8hn.png")
        await ctx.send(embed=embed)
    @commands.command()
    ...
    async def help(self,ctx):
        ...
        embed.add_field(name="%pingd", value="可以查詢當下使用embed的PING值", inline=False)
        embed.add_field(name="%invite", value="查詢邀請該機器人之網址", inline=False)
